...
class EnhancedAudioCNN110(nn.Module):
    def __init__(self):
        super(EnhancedAudioCNN110, self).__init__()
        self.conv1 = nn.Conv2d(1, 32, kernel_size=(3, 3), padding=1)
        self.bn1 = nn.BatchNorm2d(32, momentum=0.01)
        self.conv2 = nn.Conv2d(32, 64, kernel_size=(3, 3), padding=1)
        self.bn2 = nn.BatchNorm2d(64, momentum=0.1)
...
class EnhancedAudioCNN15(nn.Module):
    def __init__(self):
        super(EnhancedAudioCNN15, self).__init__()
        self.conv1 = nn.Conv2d(1, 32, kernel_size=(3, 3), padding=1)
        self.bn1 = nn.BatchNorm2d(32, momentum=0.01)
        self.conv2 = nn.Conv2d(32, 64, kernel_size=(3, 3), padding=1)
        self.bn2 = nn.BatchNorm2d(64, momentum=0.1)
    ...
    def forward(self, x):
        ...
        x = self.dropout2(x)
        x = x.view(-1, 64 * 1 * 5)  # Adjust the flattening to new layer dimensions
        x = self.relu(self.fc1(x))
        x = self.dropout2(x)
        x = self.fc2(x)
        return x
